...
| harbor-android | `6a794b412b62bc45309b2ac3a08d105bad1bffce` |

Local harbor-rs checkout may be ahead (`a49c2554…`). Cenix stays on the flake pin.

## Used as-is

From harbor-android `lib/default.nix`: `mkAndroidSdk`, `mkAndroidApk`, `mkAndroidDevShell`, `mkAndroidDeviceTools` (also on devshell `PATH` for the device scripts).
...
`mkAndroidDeviceTools` (android-device enroll/verify) is consumed from the
pinned harbor-android rev, on devshell `PATH`, with Cenix-side
`CENIX_DEVICE_DEF` support in `scripts/device-smoke.sh`. The smoke script
still falls back to its inline checks when the helper is absent.
