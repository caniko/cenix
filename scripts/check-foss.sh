...
if grep -R --include='*.kt' --include='*.xml' -nE 'WebView|PlayServices|com.google.android.gms|FirebaseAnalytics|Sentry|Datadog|remote.?config' \
  "$root/android/app/src/main" | grep -v '/uniffi/' | grep -q .; then
  echo "proprietary or WebView reference" >&2
  exit 1
fi
echo "FOSS source gate passed"
