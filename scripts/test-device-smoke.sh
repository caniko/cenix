...
case "$*" in
  ...
  *"pidof"*) echo "1234" ;;
  *"dumpsys power"*) echo "  mWakefulness=${FAKE_WAKEFULNESS:-Awake}" ;;
  *"dumpsys user"*)
...
t="$(new_test_env)"  # T5c: dozing screen refuses before any install
rc=0
env CENIX_DEVICE_SERIAL=S1 CENIX_TEST_USER=10 CENIX_EXPECTED_BUILD=2026091001 \
  CENIX_APK="$t/app.apk" CENIX_TEST_APK="$t/test.apk" CENIX_FIXTURE_APKS="$t/fixture.apk" \
  CENIX_FIXTURE_ROOTS="$t" CENIX_JNILIBS_DIR="$t/jni" CENIX_EVIDENCE_DIR="$t/ev" ADB="$t/bin/adb" \
  PATH="$t/bin:$PATH" FAKE_ADB_LOG="$t/adb.log" FAKE_SERIAL=S1 \
  FAKE_DEVICE=mustang FAKE_BUILD=2026091001 FAKE_TEST_TARGET=com.caniko.cenix \
  FAKE_USERS="10" FAKE_FOREGROUND="10" FAKE_WAKEFULNESS=Dozing \
  FAKE_INSTALLED="10" FAKE_AAPT_PACKAGE=com.caniko.cenix \
  "$dtest" >"$t/dtest.out" 2>"$t/dtest.err" || rc=$?
if [[ "$rc" != "1" ]]; then echo "FAIL dtest-dozing rc=$rc"; failures=$((failures+1));
elif ! grep -qF "screen is Dozing" "$t/dtest.err"; then echo "FAIL dtest-dozing message"; cat "$t/dtest.err"; failures=$((failures+1));
elif grep -qE '(^| )install( |$)' "$t/adb.log" 2>/dev/null; then echo "FAIL dtest-dozing installed while dozing"; failures=$((failures+1));
else echo "pass dtest-dozing"; fi; rm -rf "$t"
